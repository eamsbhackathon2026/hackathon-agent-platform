...
func (s *Service) Resolve(ctx context.Context, agentID uuid.UUID) (outbound.ToolSet, error) {
	...
	for _, tool := range catalog.HTTPTools {
		...
		set.specs = append(set.specs, domain.ToolSpec{Name: name, Description: tool.Description, JSONSchema: domain.ToolJSONSchema(tool.Params)})
	...
	for _, server := range catalog.MCPServers {
		...
		for _, tool := range cachedTools {
			...
			set.specs = append(set.specs, domain.ToolSpec{Name: name, Description: tool.Description, JSONSchema: append([]byte(nil), tool.InputSchema...)})
			serverCopy := server
