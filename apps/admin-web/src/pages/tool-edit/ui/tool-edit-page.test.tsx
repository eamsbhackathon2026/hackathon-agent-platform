...
// The page publishes its title to the shell header, so these assertions need the real shell.
function renderInShellAt(path: string) {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  render(<QueryClientProvider client={client}><MemoryRouter initialEntries={[path]}><Routes><Route element={<AppShell />}><Route path="/tools/new" element={<ToolEditPage />} /><Route path="/tools/:toolId/edit" element={<ToolEditPage />} /></Route></Routes></MemoryRouter></QueryClientProvider>);
  return () => within(screen.getByRole("banner"));
}

afterEach(() => clearAuthSession());
describe("ToolEditPage", () => {
  it("titles the shell header from the route, not from the loaded tool", async () => {
    ...
    const banner = renderInShellAt(`/tools/${tool.id}/edit`);
    // Asserted before the tool resolves: an edit route must never show the create title.
    expect(banner().getByRole("heading", { name: "Edit tool" })).toBeInTheDocument();
    expect(await screen.findByLabelText("Display name")).toHaveValue("Orders");
    expect(banner().getByRole("heading", { name: "Edit tool" })).toBeInTheDocument();
  });

  it("titles the shell header for a new tool", () => {
    setAuthSession("token", user);
    server.use(http.get(apiUrl("/v1/api-connections"), () => jsonResponse({ items: [connection], next_cursor: null })));
    expect(renderInShellAt("/tools/new")().getByRole("heading", { name: "Create HTTP tool" })).toBeInTheDocument();
  });

  it("leaves the shell header on the section title for a member", async () => {
    setAuthSession("token", { ...user, role: "member" });
    const banner = renderInShellAt("/tools/new");
    expect(await screen.findByText("Only administrators can edit tools. Contact an administrator for help.")).toBeInTheDocument();
    expect(banner().getByRole("heading", { name: "Tools" })).toBeInTheDocument();
  });
