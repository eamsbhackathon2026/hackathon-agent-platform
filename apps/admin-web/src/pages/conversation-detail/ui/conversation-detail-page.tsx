...
import { formatDate, usePageHeader } from "@/shared/lib";
...
export function ConversationDetailPage() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <Link className="inline-flex items-center gap-2 text-sm hover:underline" to="/conversations"><ArrowLeft className="size-4" />Conversation History</Link>
      <div className="flex gap-2">{canContinue ? <Button asChild><Link to={`/playground?agent=${conversation.data.agent_id}&session=${conversation.data.id}`}><MessageSquare />Continue conversation</Link></Button> : null}<DeleteConversationButton conversationId={sessionId} onDeleted={() => navigate("/conversations")} /></div>
    </div>
