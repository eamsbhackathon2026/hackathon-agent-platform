...
import { DeleteConversationContextMenu } from "@/features/delete-conversation";
...
export function ConversationRailContent({ activeSessionId, disabled, activeDeleteDisabled, onNew, onSelect, onActiveDeleted, onNavigate }: ConversationRailContentProps) {
  ...
  // Everyone gets the full-history link: the rail only holds the newest chats, and
  // it is the one place a keyboard user can reach delete on a Mac, where no key
  // raises a context menu. The server narrows the list to the reader's own
  // conversations unless they manage the workspace, so the label follows suit.
  const seesWholeWorkspace = user ? canManageWorkspace(user.role) : false;
  ...
  if (loading) {
    ...
    listContent = <ul aria-label="Recent conversations" className="space-y-1 pr-2">{items.map((item) => {
      ...
      return <DeleteConversationContextMenu key={item.id} conversationId={item.id} ariaLabel={`Delete ${title}`} disabled={disabled || (active && activeDeleteDisabled)} disabledReason="Wait for the response to finish." onDeleted={active ? onActiveDeleted : undefined}>
        <li className={cn("flex min-w-0 items-center gap-1 rounded-xl border border-transparent p-1 transition-colors", active ? "border-primary/30 bg-primary/5" : "hover:bg-accent/70")}>
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3">
    ...
    {user ? <Link to="/conversations" aria-disabled={disabled || undefined} className={cn("inline-flex min-h-11 items-center justify-center rounded-xl px-3 text-sm font-medium text-primary underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40", disabled && "pointer-events-none opacity-45")} onClick={(event) => { if (disabled) event.preventDefault(); else onNavigate?.(); }}>{seesWholeWorkspace ? "View workspace conversations" : "View all your conversations"}</Link> : null}
  </div>;
}
