...
export function HttpToolBuilder({ initialParams = [], method = "POST", onChange }: Props) {
  ...
  return <div className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-medium">Assistant inputs</h3><p className="text-sm text-muted-foreground">Define each value with a clear name and description.</p><p className="text-sm text-muted-foreground">Checking "Show in progress" lets the customer watching the assistant work see this input's value while the tool runs. Leave it unchecked for anything sensitive, such as an account number or an amount.</p></div><Button type="button" variant="outline" onClick={() => update([...rows, { rowId: crypto.randomUUID(), name: "", type: "string", description: "", required: false, in: "query", show_in_progress: false }])}><Plus /> Add row</Button></div>
    {rows.map((row, index) => <div className="grid gap-2 rounded-lg border p-3 md:grid-cols-[1fr_1fr_0.8fr_1fr_auto_auto_auto]" key={row.rowId}>
      ...
      <Label className="flex items-center gap-2" title="Shown to the customer while the assistant runs this tool. Do not enable for sensitive values."><Checkbox checked={row.show_in_progress} onCheckedChange={(checked) => update(rows.map((item) => item.rowId === row.rowId ? { ...item, show_in_progress: checked === true } : item))} />Show in progress</Label>
